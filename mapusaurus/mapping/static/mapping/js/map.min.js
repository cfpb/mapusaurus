...
function layerUpdate(layer) {
    ...
    addParam("category", layer), updateCircles(layerType);
}
...
$(document).ready(function() {
    ...
    layerUpdate(cat), $(catId).addClass("active")) : (assignCat("sequential3"), layerUpdate(cat), 
    ...
    categoryOptions.on("click", function() {
        ...
        assignCat(selectedOption.attr("id")), selectedOption.addClass("active"), layerUpdate(cat);
    ...
    togglePeers(peerStatus)) : addParam("peers", !1), $("#peerSelect").change(function() {
        ...
        console.log("gqt in moveend: ", geoQueryType), moveEndAction[geoQueryType]();
    }), updatePrintLink(), updateCensusLink(), layerUpdate($(catId).val()), $(window).on("hashchange", function() {
...
moveEndAction.selected = function() {
    ...
    buildKeyCircles()) : (initCalls(geoQueryType), oldEndAction = "selected", buildKeyCircles());
    ...
    "all_msa" === oldEndAction ? $.when(getMsasInBounds()).done(function(data) {
        ...
        buildKeyCircles();
    }) : (initCalls(geoQueryType), oldEndAction = "all_msa", buildKeyCircles());
}, moveEndAction.all = function() {
    initCalls(geoQueryType), oldEndAction = "all", buildKeyCircles();
};

...
function plotLarVolume() {
    var margin = {
        ...
        bottom: 30,
        left: 40
    }, width = 1180 - margin.left - margin.right, height = 200 - margin.top - margin.bottom, data = (width / larVolume.length, 
    _.zip(pctMinority, larVolume));
    data = _.sortBy(data, function(item) {
        return item[0];
    }), data = _.zip.apply(_, data), pctMinority = data[0], larVolume = data[1];
    var colorMap = d3.scale.quantile().domain([ 0, .5, .8, 1 ]).range([ "#E8E7E3", "#B7C8D6", "#7FA2BB" ]);
    d3.select("#plot-container").selectAll("*").remove();
    var larSvg = d3.select("#plot-container").append("svg").attr("width", width).attr("height", height), minSvg = d3.select("#plot-container").append("svg").attr("width", width).attr("height", height), larChart = larSvg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"), minChart = minSvg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"), larX = d3.scale.ordinal().rangeRoundBands([ 0, width ], .1), larY = d3.scale.linear().range([ height, 0 ]), larYAxis = (d3.svg.axis().scale(larX).orient("bottom"), 
    d3.svg.axis().scale(larY).orient("left")), minX = d3.scale.ordinal().rangeRoundBands([ 0, width ], .1), minY = d3.scale.linear().range([ height, 0 ]), minYAxis = (d3.svg.axis().scale(minX).orient("bottom"), 
    d3.svg.axis().scale(minY).orient("left"));
    larX.domain(geoIds), larY.domain([ 0, d3.max(larVolume, function(d) {
        return d;
    }) ]), minX.domain(geoIds), minY.domain([ 0, d3.max(pctMinority, function(d) {
        return d;
    }) ]), larChart.append("g").attr("class", "y axis").call(larYAxis).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".5em").style("text-anchor", "end").text("LAR per 1000 households"), 
    larChart.selectAll(".bar-lar").data(larVolume).enter().append("rect").attr("class", "bar-lar").attr("x", function(d, i) {
        return larX(geoIds[i]);
    }).attr("width", larX.rangeBand()).attr("y", function(d) {
        return larY(d);
    }).attr("height", function(d) {
        return height - larY(d);
    }).style("fill", function(d, i) {
        return colorMap(pctMinority[i]);
    }), minChart.append("g").attr("class", "y axis").call(minYAxis).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", ".5em").style("text-anchor", "end").text("Minority Percentage"), 
    minChart.selectAll(".bar-min").data(pctMinority).enter().append("rect").attr("class", "bar-min").attr("x", function(d, i) {
        return minX(geoIds[i]);
    }).attr("width", larX.rangeBand()).attr("y", function(d) {
        return minY(d);
    }).attr("height", function(d, i) {
        return height - larY(larVolume[i]) > 0 ? height - minY(d) : 0;
...
>>>>>>> Generate tables with multiple templates
}
...
<<<<<<< HEAD
    params.swLat = bounds.swLat, params.swLon = bounds.swLon), geoType && (params.geoType = geoType);
=======
    params.swLat = bounds.swLat, params.swLon = bounds.swLon), console.log("Bounds: ", bounds), 
    console.log("Param Bounds: ", params.bounds), geoType && (params.geoType = geoType);
>>>>>>> updated init functions to handle msa clipping cases
...
<<<<<<< HEAD
    dataStore.tracts = {}, larVolume = [], pctMinority = [], geoIds = [], _.each(rawGeo.features, function(feature) {
        var geoid = feature.properties.geoid;
        geoIds.push(geoid), dataStore.tracts[geoid] = feature.properties, _.extend(dataStore.tracts[geoid], rawData.minority[geoid]), 
...
function buildKeyCircles() {
    ...
    0), rad = 0, maxRad = _.max(circles, function(circleObj) {
        ...
        var circle = circles[i];
        rad = circle._radius, posx += 45, svgStr += '<circle cx="' + posx + '" cy="' + (posy - rad) + '" r="' + rad + '" fillColor="#111111" fill-opacity=".7" stroke=true color="#333", opacity=1/>', 
        svgStr += '<text x="' + posx + '" y="' + textPosy + '" font-size="1em" text-anchor="middle">' + circle.volume + "</text>";
    }
    svgStr += "</svg>", selector.html(svgStr);
}
...
function getRange(data) {
    ...
    return max.volume < 2 && max.volume > 0 ? valArray = [ max ] : 0 === max.volume ? valArray = [] : (0 === min.volume ? (min = {
        volume: 1
    }, drawNewArray = [ min.volume, min.volume + multiple, max.volume ], _.each(drawNewArray, function(val) {
        addKeyLayerCircle(val);
    }), keyCircles = _.matches({
        keyCircle: 1
    }), keyCirclesFilter = _.filter(data, keyCircles), valArray = [ keyCirclesFilter[0], keyCirclesFilter[1], max ]) : (_.each(drawNewArray, function(val) {
        addKeyLayerCircle(val);
    }), keyCircles = _.matches({
        keyCircle: 1
    }), keyCirclesFilter = _.filter(data, keyCircles), valArray = [ min, keyCirclesFilter[0], max ]), 
...
<<<<<<< HEAD
var cat, catId, geoQueryType = "selected";
=======
<<<<<<< HEAD
...
=======
    ...
    }), "undefined" != typeof loadParams.geo_query_type ? (console.log("geoQueryType: ", loadParams.geo_query_type.values), 
    geoQueryType = loadParams.geo_query_type.values) : addParam("geo_query_type", "selected"), 
    $("#geoQueryTypeSelect").change(function() {
        var el = $("#gepQueryTypeSelect");
        geoQueryType = el.prop("selected").val(), addParam("geo_query_type", geoQueryType);
>>>>>>> updated init functions to handle msa clipping cases
...
<<<<<<< HEAD
    ...
    }), updatePrintLink(), updateCensusLink(), layerUpdate(cat), $(window).on("hashchange", function() {
...
=======
    ...
    updateCensusLink(), layerUpdate($("#category-selector").val()), $(window).on("hashchange", function() {
        updatePrintLink(), updateCensusLink(), initCalls(geoQueryType);
    }), initCalls(geoQueryType);
});

<<<<<<< HEAD
var rawGeo, rawLar, rawMinority, rawData, larVolume, pctMinority, geoIds, isUIBlocked = !1, dataStore = {};
=======
var msaArray = [], moveEndAction = {};

moveEndAction.selected = function() {}, moveEndAction.all_msa = function() {
    var oldMsaArray = msaArray.slice(0);
    $.when(getMsasInBounds()).done(function(data) {
        var intersect = _.intersection(oldMsaArray, data);
        intersect.length !== oldMsaArray.length ? initCalls(geoQueryType) : data.length !== intersect.length && initCalls(geoQueryType);
    });
}, moveEndAction.all = function() {
    initCalls(geoQueryType);
};

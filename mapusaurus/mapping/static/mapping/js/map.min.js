...
function togglePeers(status) {
    status ? ($("#lender-peers-list").removeClass("hidden"), $("#lender-peers").addClass("green-highlight")) : ($("#lender-peers-list").addClass("hidden"), 
    $("#lender-peers").removeClass("green-highlight")), addParam("peers", status), $("#peerSelect").prop("checked", status);
}
...
function getTractData(bounds, actionTakenVal) {
    ...
    urlParam("year") && (params.year = urlParam("year")), urlParam("lender") ? (params.lender = urlParam("lender"), 
...
window.console || (console = {
    ...
    if ($("#superSelect").change(function() {
        var el = $("#superSelect"), status = el.prop("checked");
        toggleSuper(status), $("#peerSelect").prop("disabled", status), init();
    }), "undefined" != typeof loadParams.branches) {
        var status = "true" === loadParams.branches.values;
        $("#branchSelect").prop("checked", status), toggleBranches(status);
    } else addParam("branches", !1);
    if ($("#branchSelect").change(function() {
        var el = $("#branchSelect"), status = el.prop("checked");
        toggleBranches(status);
    }), "undefined" != typeof loadParams.peers) {
        var status = "true" === loadParams.peers.values;
        $("#peerSelect").prop("checked", status), togglePeers(status), $("#superSelect").prop("disabled", status);
    } else addParam("peers", !1);
    $("#peerSelect").change(function() {
        var el = $("#peerSelect"), status = el.prop("checked");
        togglePeers(status), init();
    ...
    }), map.on("moveend", _.debounce(init, 500)), init();
});

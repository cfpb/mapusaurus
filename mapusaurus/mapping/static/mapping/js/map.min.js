...
function toggleSuper(status) {
    ...
    status ? $("#download-data").attr("href", url) : $("#download-data").attr("href", origUrl), 
...
function updateCircles(layerType) {
    layers.Centroids.eachLayer(function(layer) {
        if ("minority" == layerType) {
            ...
            layer.setStyle(newStyle), console.log("baseStyle: ", baseStyle);
        } else "seq" == layerType && (layer.setStyle(seqBaseStyle), console.log(" Sequence Base Used in Update Circles"));
    }), console.log("color update complete.");
}

function drawCircle(geo, layerType) {
    var style, data = geo;
    style = "seq" === layerType ? seqBaseStyle : minorityContinuousStyle(geo, baseStyle), 
...
function layerUpdate(layer) {
    ...
    var layer = layerEval.layer, layerType = layerEval.type;
    map.addLayer(layer), layer.bringToFront(), layers.Water.bringToFront(), layers.Boundaries.bringToFront(), 
    layers.MSALabels.bringToFront(), addParam("category", $("#category-selector option:selected").val()), 

...
function buildKeyCircles() {
    ...
    selector.html("");
    for (var svgStr = '<svg height="75px">', circles = getRange(map._layers), $scale = $("#action-taken-selector option:selected"), posx = ($scale.data("scale"), 
    0), rad = 0, maxRad = _.max(circles, function(circleObj) {
        return circleObj._radius;
    })._radius, posy = 2 * maxRad, textPosy = posy + 16, i = 0; i < circles.length; i++) {
        var circle = circles[i];
        rad = circle._radius, console.log("RAD: ", rad), posx += 45, svgStr += '<circle cx="' + posx + '" cy="' + (posy - rad) + '" r="' + rad + '" fill="gray" anchor="bottom"/>', 
...
function getRange(data) {
    var circles = _.matches({
        type: "tract-circle"
    }), max = (_.filter(data, circles), _.max(data, function(circleObj) {

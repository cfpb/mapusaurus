...
function init() {
    blockStuff(), $.when(getTractsInBounds(getBoundParams()), getTractData(getBoundParams(), getActionTaken($("#action-taken-selector option:selected").val()))).done(function(data1, data2) {
        ...
        plotLarVolume(), $.unblockUI(), isUIBlocked = !1;
    });
}
...
function plotLarVolume() {
    {
        var margin = {
            ...
            bottom: 30,
            left: 40
        }, width = 1180 - margin.left - margin.right, height = 200 - margin.top - margin.bottom;
        width / larVolume.length;
    }
    ...
    }).attr("width", minX.rangeBand()).attr("y", function(d) {
        return minY(d);
    }).attr("height", function(d) {

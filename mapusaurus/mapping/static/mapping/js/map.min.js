    ...
    blockStuff(), $.when(getTractsInBounds(getBoundParams()), getTractData(getBoundParams(), getActionTaken($("#action-taken-selector option:selected").val()))).done(function(data1, data2) {
        {
            ...
            getLayerType(hashInfo.category.values);
        }
        rawGeo = data1[0], rawData = data2[0], createTractDataObj(), updateVoronoi(rawGeo), 
...
function randomPoint(rBounds) {
    var point = new Array(2), west = rBounds.getNorth(), north = rBounds.getEast(), east = rBounds.getSouth(), south = rBounds.getWest();
    return point[0] = west + Math.random() * (east - west), point[1] = north + Math.random() * (south - north), 
    point;
}

function makeDots(polygons, features) {
    for (var points = [], i = 0; i < polygons.length; i++) for (var poly = polygons[i], reverseBounds = L.latLngBounds(poly), j = 0, len = features[i].properties.volume; len > j; j++) {
        for (var pt = randomPoint(reverseBounds), bail = 0; !(pointInPoly(pt, poly) || ++bail > 10); ) pt = randomPoint(reverseBounds);
        points.push(pt);
    }
    return points;
}
...
function pointInPoly(point, vs) {
    for (var x = point[0], y = point[1], inside = !1, i = 0, j = vs.length - 1; i < vs.length; j = i++) {
        var xi = vs[i][0], yi = vs[i][1], xj = vs[j][0], yj = vs[j][1], intersect = yi > y != yj > y && (xj - xi) * (y - yi) / (yj - yi) + xi > x;
...
function initVoronoi() {
    map._initPathRoot();
    var svg = d3.select("#map").select("svg"), g = svg.append("g").attr("class", "leaflet-zoom-hide"), voronoi = d3.geom.voronoi();
    return function(collection) {
        {
            var positions = [], features = collection.features;
            map.getSize();
        }
        features.forEach(function(d) {
            var latlng = new L.LatLng(d.properties.centlat, d.properties.centlon), point = map.latLngToLayerPoint(latlng);
            positions.push([ point.x, point.y ]);
        });
        var polygons = voronoi(positions);
        window.poly = polygons, window.feat = features;
        var points = makeDots(polygons, features);
        g.selectAll("circle").data(points).enter().append("circle").attr("class", "densityDot").attr({
            cx: function(d) {
                return d[0];
            },
            cy: function(d) {
...
$(document).ready(function() {
    ...
    $(".tabs").show(), $(window).resize(function() {
        setMapHeight();
    }), updateVoronoi = initVoronoi(), $("#category-selector").on("change", function() {
    ...
    }), map.on("moveend", _.debounce(init, 500)), map.on("movestart", function() {
        d3.selectAll(".densityDot").remove();
    }), updatePrintLink(), updateCensusLink(), layerUpdate($("#category-selector").val()), 

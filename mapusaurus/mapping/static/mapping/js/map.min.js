...
function drawCircle(geo, options) {
    ...
    circle.keyCircle = 0, circle.geoid = circle.geoid.replace(new RegExp("^" + selectedYear), ""), 
    "undefined" != typeof options && (circle.keyCircle = options.keyCircle), circle.on("mouseover mousemove", function(e) {
        new L.Rrose({
...
function getPeerLinks() {
    ...
    $.each(peerList, function(i, val) {
        var instid = $(val).data("instid"), href = "/map/?metro=" + urlParam("metro") + "&lender=" + instid + "&year=" + selectedYear + window.location.hash.replace("&peers=true", "");
        $(val).attr("href", href);
    });
}
...
var rawGeo, rawLar, rawMinority, rawData, selectedYear = selectedYear || 2014, isUIBlocked = !1, larVolume = [], pctMinority = [], dataStore = {};

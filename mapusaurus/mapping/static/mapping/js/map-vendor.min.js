...
function updateUrlHash(params) {
    function buildHashParam(param, name) {
        console.log("name: ", name), console.log("param: ", param), hashParams.push(name + "=" + param.values);
    }
    var newHash, hashParams = [];
    console.log("params inside updateUrlHash: ", params), _.forEach(params, buildHashParam), 
    newHash = "&" + hashParams.join("&"), window.location.hash = newHash;
}
...
(function() {
    ...
    L.Hash = function(map) {
        ...
        var params = getHashParams();
        if (console.log("params zoom: ", params.zoom), "undefined" != typeof params.zoom || "undefined" != typeof params.lat || "undefined" != typeof params.lon) {
        ...
        update: function() {
            ...
            parsed ? (this.movingMap = !0, this.map.setView(parsed.center, parsed.zoom), this.movingMap = !1) : (console.log("Update else calling onMapMove"), 
        ...
        startListening: function() {
            console.log("startListening calling onMapMove"), this.map.on("moveend", this.onMapMove, this), 
            HAS_HASHCHANGE ? L.DomEvent.addListener(window, "hashchange", this.onHashChange) : (clearInterval(this.hashChangeInterval), 
            this.hashChangeInterval = setInterval(this.onHashChange, 500)), this.isListening = !0;
        },
        stopListening: function() {
            console.log("stopListening calling onMapMove"), this.map.off("moveend", this.onMapMove, this), 
            HAS_HASHCHANGE ? L.DomEvent.removeListener(window, "hashchange", this.onHashChange) : clearInterval(this.hashChangeInterval), 
            this.isListening = !1;
        }
    }, L.hash = function(map) {

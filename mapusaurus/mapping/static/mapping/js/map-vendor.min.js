...
(function() {
    ...
    L.Hash = function(map) {
        ...
        return console.log("RETURN OBJ: ", returnObj), console.log("LAT, LON: ", returnObj.lat, " - ", returnObj.lon), 
        returnObj;
    }, L.Hash.prototype = {
        map: null,
        lastHash: null,
        ...
        init: function(map) {
            console.log("INIT"), this.map = map, this.lastHash = null, this.onHashChange(), 
            this.isListening || this.startListening();
        },
        removeFrom: function() {
            console.log("REMOVE FROM"), this.changeTimeout && clearTimeout(this.changeTimeout), 
            this.isListening && this.stopListening(), this.map = null;
        },
        onMapMove: function() {
...
=======
            if (console.log("UPDATE RUN"), _.isEqual(hash, this.lastHash)) return !1;
            console.log("Hash: ", hash);
>>>>>>> updated init functions to handle msa clipping cases
            var parsed = this.parseHash(hash);
            parsed ? (console.log("IF PARSED SET VIEW"), this.movingMap = !0, this.map.setView(parsed.center, parsed.zoom), 
            this.movingMap = !1) : (console.log("ON MAP MOVE THIS.MAP"), this.onMapMove(this.map));
        },
        changeDefer: 200,
<<<<<<< HEAD
        changeTimeout: 200,
...
>>>>>>> updated init functions to handle msa clipping cases
        ...
        startListening: function() {
            console.log("START LISTENING"), this.map.on("moveend", this.onMapMove, this), HAS_HASHCHANGE ? L.DomEvent.addListener(window, "hashchange", this.onHashChange) : (clearInterval(this.hashChangeInterval), 
        ...
        stopListening: function() {
            ...
            this.isListening = !1, console.log("STOP LISTENING");
        }
    }, L.hash = function(map) {
...
}(window), L.Util.ajax = function(url, cb) {
    ...
    initialize: function(url, options) {
        L.Util.setOptions(this, options), this._url = url, this._cache = {};
        for (var i = 0; window["lu" + i]; ) i++;
        this._windowKey = "lu" + i, window[this._windowKey] = {};
        var subdomains = this.options.subdomains;
        "string" == typeof this.options.subdomains && (this.options.subdomains = subdomains.split(""));
    },
    onAdd: function(map) {
        this._map = map, this._container = this._map._container, this._update();
        var zoom = this._map.getZoom();
        zoom > this.options.maxZoom || zoom < this.options.minZoom || (map.on("click", this._click, this), 
        map.on("mousemove", this._move, this), map.on("moveend", this._update, this));
    },
    onRemove: function() {
        var map = this._map;
        map.off("click", this._click, this), map.off("mousemove", this._move, this), map.off("moveend", this._update, this), 
        this.options.pointerCursor && (this._container.style.cursor = "");
    },
    redraw: function() {
        this._request_queue = [];
        for (var req_key in this._requests) this._requests.hasOwnProperty(req_key) && this._abort_request(req_key);
    ...
    _move: function(e) {
        ...
        on.data !== this._mouseOn ? (this._mouseOn && (this.fire("mouseout", {
            latlng: e.latlng,
            data: this._mouseOn
        }), this.options.pointerCursor && (this._container.style.cursor = "")), on.data && (this.fire("mouseover", on), 
        this.options.pointerCursor && (this._container.style.cursor = "pointer")), this._mouseOn = on.data) : on.data && this.fire("mousemove", on);
    },
    _objectForEvent: function(e) {
        var map = this._map, point = map.project(e.latlng), tileSize = this.options.tileSize, resolution = this.options.resolution, x = Math.floor(point.x / tileSize), y = Math.floor(point.y / tileSize), gridX = Math.floor((point.x - x * tileSize) / resolution), gridY = Math.floor((point.y - y * tileSize) / resolution), max = map.options.crs.scale(map.getZoom()) / tileSize;
        x = (x + max) % max, y = (y + max) % max;
        var data = this._cache[map.getZoom() + "_" + x + "_" + y];
        if (!data || !data.grid) return {
            latlng: e.latlng,
            data: null
        };
        var idx = this._utfDecode(data.grid[gridY].charCodeAt(gridX)), key = data.keys[idx], result = data.data[key];
        return data.data.hasOwnProperty(key) || (result = null), {
            latlng: e.latlng,
            data: result
        };
    },
    _update: function() {
        ...
        if (!(zoom > this.options.maxZoom || zoom < this.options.minZoom)) {
            for (var nwTilePoint = new L.Point(Math.floor(bounds.min.x / tileSize), Math.floor(bounds.min.y / tileSize)), seTilePoint = new L.Point(Math.floor(bounds.max.x / tileSize), Math.floor(bounds.max.y / tileSize)), max = this._map.options.crs.scale(zoom) / tileSize, visible_tiles = [], x = nwTilePoint.x; x <= seTilePoint.x; x++) for (var y = nwTilePoint.y; y <= seTilePoint.y; y++) {
                var xw = (x + max) % max, yw = (y + max) % max, key = zoom + "_" + xw + "_" + yw;
                visible_tiles.push(key), this._cache.hasOwnProperty(key) || (this._cache[key] = null, 
                this.options.useJsonP ? this._loadTileP(zoom, xw, yw) : this._loadTile(zoom, xw, yw));
            }
            for (var req_key in this._requests) visible_tiles.indexOf(req_key) < 0 && this._abort_request(req_key);
        }
    },
    _loadTileP: function(zoom, x, y) {
        var head = document.getElementsByTagName("head")[0], key = zoom + "_" + x + "_" + y, functionName = "lu_" + key, wk = this._windowKey, self = this, url = L.Util.template(this._url, L.Util.extend({
        ...
        window[wk][functionName] = function(data) {
            self._cache[key] = data, delete window[wk][functionName], head.removeChild(script), 
    ...
    _loadTile: function(zoom, x, y) {
        var url = L.Util.template(this._url, L.Util.extend({
            ...
            x: x,
            y: y
        }, this.options)), key = zoom + "_" + x + "_" + y, self = this;
        this._queue_request(key, function() {
            return L.Util.ajax(url, function(data) {
                self._cache[key] = data, self._finish_request(key);
            });
        });
    },
    _queue_request: function(key, callback) {
        this._requests[key] = {
            ...
            timeout: null,
            handler: null
        }, this._request_queue.push(key), this._process_queued_requests();
    },
    _finish_request: function(key) {
        var pos = this._requests_in_process.indexOf(key);
        pos >= 0 && this._requests_in_process.splice(pos, 1), pos = this._request_queue.indexOf(key), 
        pos >= 0 && this._request_queue.splice(pos, 1), this._requests[key] && (this._requests[key].timeout && window.clearTimeout(this._requests[key].timeout), 
        delete this._requests[key]), this._process_queued_requests();
    },
    _abort_request: function(key) {
        this._requests[key] && this._requests[key].handler && "function" == typeof this._requests[key].handler.abort && this._requests[key].handler.abort(), 
        null === this._cache[key] && delete this._cache[key], this._finish_request(key);
    },
    ...
    _process_request: function(key) {
        ...
        }, this.options.requestTimeout), this._requests_in_process.push(key);
        var handler = this._requests[key].callback();
        this._requests[key] && (this._requests[key].handler = handler);
    },
    _utfDecode: function(c) {
        return c >= 93 && c--, c >= 35 && c--, c - 32;
    }

...
function createTable(showPeers) {
    msaData || (msaData = getTableData()), console.log("MSA DATA: ", msaData), msaData.done(function(res) {
        console.log("msaData is done"), tableData || (tableData = res, prepTableData(tableData));
...
function buildRow(data, rowType, showPeers) {
    ...
    return templateData = _.extend({
        rowType: rowType,
        showPeers: showPeers
    }, data), templateData.className = rowType.toLowerCase() + "-row ", templateData.className += data.isPeer ? " peer-row tablesorter-childRow" : " target-row", 
    rowTemplate(templateData);
}

function activateTable($tbl) {
    return console.log("Table Sorter...."), $tbl.tablesorter({
        headerTemplate: "",
...
function destroyData() {
    "chart-toggle__lar-chart" === currentChart ? (destroyLarChart(), $("#chart-container").hide()) : (destroyTable(), 
    $("#table-container").hide());
}

function destroyTable() {
    $(".summary-data-table").trigger("destroy").remove();
}

function toggleDataContainer(showData) {
    showDataContainer = showData, showData ? ($("#table-container").show(), $("#data-container").show()) : ($("#table-container").hide(), 
    $("#data-container").hide()), setMapHeight();
}
